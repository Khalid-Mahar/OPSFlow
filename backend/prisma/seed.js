...
async function main() {
    ...
    const teamLeadPerms = allPerms.filter(p => ['employee.view', 'asset.view', 'sim.view', 'project.view', 'project.create', 'project.edit', 'task.view', 'task.create', 'task.edit', 'file.view', 'file.upload'].includes(`${p.module}.${p.action}`));
    ...
    const employeePerms = allPerms.filter(p => ['employee.view', 'project.view', 'task.view', 'task.edit', 'file.view', 'file.upload'].includes(`${p.module}.${p.action}`));
    ...
    const superAdmin = await prisma.user.upsert({
        ...
        update: {},
        create: { email: 'superadmin@opsflow.com', password: await bcryptjs_1.default.hash('Admin@2024', 12), name: 'Super Administrator', roleId: superAdminRole.id, companyId: company.id, status: 'ACTIVE' }
    });
    const admin = await prisma.user.upsert({
        where: { email: 'admin@opsflow.com' },
        update: {},
        create: { email: 'admin@opsflow.com', password: await bcryptjs_1.default.hash('Admin@2024', 12), name: 'IT Administrator', roleId: adminRole.id, companyId: company.id, status: 'ACTIVE' }
    });
    const teamLead = await prisma.user.upsert({
        where: { email: 'teamlead@opsflow.com' },
        update: {},
        create: { email: 'teamlead@opsflow.com', password: await bcryptjs_1.default.hash('Employee@2024', 12), name: 'Ahmed Raza', roleId: teamLeadRole.id, companyId: company.id, status: 'ACTIVE' }
    });
    const sara = await prisma.user.upsert({
        where: { email: 'sara@opsflow.com' },
        update: {},
        create: { email: 'sara@opsflow.com', password: await bcryptjs_1.default.hash('Employee@2024', 12), name: 'Sara Khan', roleId: employeeRole.id, companyId: company.id, status: 'ACTIVE' }
    });
    const ali = await prisma.user.upsert({
        where: { email: 'ali@opsflow.com' },
        update: {},
        create: { email: 'ali@opsflow.com', password: await bcryptjs_1.default.hash('Employee@2024', 12), name: 'Ali Hassan', roleId: employeeRole.id, companyId: company.id, status: 'ACTIVE' }
    });
    const fatima = await prisma.user.upsert({
        where: { email: 'fatima@opsflow.com' },
        update: {},
        create: { email: 'fatima@opsflow.com', password: await bcryptjs_1.default.hash('Employee@2024', 12), name: 'Fatima Malik', roleId: employeeRole.id, companyId: company.id, status: 'ACTIVE' }
    });
    ...
    const encryptPw = (pw) => CryptoJS.AES.encrypt(pw, process.env.ENCRYPTION_KEY || 'opsflow-aes-256-secret-key-32chars!!').toString();
